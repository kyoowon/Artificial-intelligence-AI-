...
class MCTS:
    ...
    def _expand(self, node):    # 확장 단계
        "childen에 node의 자식노드 추가"
        if node in self.childen:
            return  # 이미 확장된 노드
        self.childen[node] = node.find_children()   # 선택가능 move들을 node의 children에 추가
...
class TicTacToeBoard(TTTB, Node):   # TTTB의 속성들도 상속
    ...
    def display_board(board):   # 보드의 상태 출력
