...
# 퍼지 규칙을 적용한 food quality가 qual_val, service 정수가 serv_val 일 때 tip 계산
def compute_tip_amout(qual_val, serv_val):
	...
	#각 규칙의 추론결과 결합
	aggregated = np.fmax(tip_activation_low, np.fmax(tip_acitvation_medium, tip_activation_high))

	# 비퍼지화
	tip = fuzz.defuzz(x_tip, aggregated, 'centroid')
	return tip

print('food quality score = 6.6, service score = 9일 때 팁 : ', compute_tip_amout(6.6, 9))
...
d_serv = np.arange(0, 10, 0.5) # 서비스 만족도의 범위
...
for i in range(20):
	for j in range(20):
		T[i, j] = compute_tip_amout(Q[i, j], S[i, j])
...
ax.set_zlabel('tip')
ax.set_zlim(0, 30)
ax.set_title('fussy inference-based tip computation')
